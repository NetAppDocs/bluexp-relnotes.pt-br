...
7 de outubro de 2024 - Página da lista de alertas do BlueXP  você pode identificar rapidamente clusters do ONTAP com baixa capacidade ou baixo desempenho, avaliar a extensão da disponibilidade e identificar riscos de segurança. Você pode visualizar alertas relacionados a capacidade, performance, proteção, disponibilidade, segurança e configuração.

Você pode detalhar os detalhes do alerta e encontrar recomendações.

Você pode visualizar os BlueXP  alertas associados ao ambiente de armazenamento do ONTAP e detalhar os detalhes vinculados ao Gerenciador de sistema do ONTAP.

https://docs.netapp.com/us-en/bluexp-alerts/concept-alerts.html["Saiba mais sobre os alertas do BlueXP"].

O Amazon FSX for ONTAP

02 de fevereiro de 2025 associe o sistema de arquivos FSX for ONTAP com um espaço de trabalho no BlueXP  após a integração do BlueXP  em novembro de 2024, os sistemas de arquivos FSX for ONTAP recém-criados não estavam associados a um workspace no BlueXP . Agora, quando você cria ou descobre os sistemas de arquivos FSX for ONTAP, eles estão associados a um espaço de trabalho em uma conta do BlueXP .
...
Remoção do sistema de arquivos do BlueXP  Canvas agora você pode remover um sistema de arquivos FSX for ONTAP de um espaço de trabalho na tela BlueXP . Esta operação dissocia o sistema de arquivos de um workspace para que você possa associá-lo a outro workspace dentro da mesma conta do BlueXP .
...
O CloudShell está disponível nas cargas de trabalho do BlueXP 
...
06 de janeiro de 2025 o NetApp lança recursos adicionais do CloudFormation o NetApp agora fornece recursos do CloudFormation que permitem que os clientes utilizem componentes avançados do ONTAP que não são expostos no console da AWS. O CloudFormation é o mecanismo de infraestrutura como código para a AWS. Você poderá criar relacionamentos de replicação, compartilhamentos CIFS, políticas de exportação NFS, snapshots e muito mais.
...
11 de novembro de 2024 o FSX for ONTAP integra-se com armazenamento na fábrica de carga de trabalho do BlueXP  as tarefas de gerenciamento do sistema de arquivos do FSX for ONTAP, como adicionar volumes, expandir a capacidade do sistema de arquivos e gerenciar VMs de armazenamento, agora são gerenciadas na fábrica de carga de trabalho do BlueXP , um novo serviço oferecido pelo NetApp e pelo Amazon FSX for NetApp ONTAP. Você pode usar suas credenciais e permissões existentes como antes. A diferença é que agora você pode fazer mais da fábrica de workloads do BlueXP  para gerenciar seus sistemas de arquivos. Quando você abre um ambiente de trabalho do FSX for ONTAP a partir do BlueXP  Canvas, você vai diretamente para a fábrica de cargas de trabalho do BlueXP .
...
5 de Junho de 2023 :Icons: Font :imagesdir: ../media/
...
Recursos de rede agora suportados no BlueXP  ao configurar um volume no Azure NetApp Files a partir do BlueXP , agora você pode indicar recursos de rede. Isso se alinha à funcionalidade disponível no Azure NetApp Files nativo.

12 de junho de 2024 Nova permissão necessária
...
22 de abril de 2024 - modelos de volume não são mais suportados
...
22 de novembro de 2024 esta versão de backup e recuperação do BlueXP  inclui as seguintes atualizações.
...
27 de setembro de 2024 esta versão de backup e recuperação do BlueXP  inclui as seguintes atualizações.

Suporte de Podman no RHEL 8 ou 9 com Browse and Restore BlueXP  backup e recuperação agora oferece suporte a restauração de arquivos e pastas no Red Hat Enterprise Linux (RHEL) versões 8 e 9 usando o mecanismo Podman. Isso se aplica ao método de pesquisa e restauração de backup e recuperação do BlueXP .
...
Esta versão inclui um índice de catálogo melhorado que completa a indexação da linha de base muito mais rapidamente. A indexação mais rápida permite que você use o recurso Pesquisa e Restauração mais rapidamente.
...
Integração com o AWS IAM Roles Anywhere
...
Agora disponível anteriormente, os volumes FlexVol podem ser restaurados, mas você não pode restaurar pastas ou diretórios FlexGroup FlexGroup. Com o ONTAP 9.15,1 P2, você pode restaurar pastas do FlexGroup usando a opção Procurar e restaurar.
...
16 de dezembro de 2024 versão 1,38 esta versão de classificação do BlueXP  inclui melhorias gerais e correções de bugs.

4 de novembro de 2024 versão 1,37 esta versão de classificação BlueXP  inclui as seguintes atualizações.
...
10 de fevereiro de 2025 Autenticação de usuário habilitada para acessar o Gerenciador de sistema do BlueXP  como administrador do BlueXP , agora você pode ativar a autenticação para usuários do ONTAP acessando o Gerenciador de sistema do ONTAP a partir do BlueXP . Pode ativar esta opção editando as definições do conetor BlueXP . Esta opção está disponível para os modos padrão e privado.
...
A opção de gerenciamento avançado do Cloud Volumes ONTAP a partir do BlueXP  através do Gerenciador de sistema do BlueXP  ONTAP foi renomeada de *Visão Avançada* para *Gerenciador de sistema*.

link:https://docs.netapp.com/us-en/bluexp-cloud-volumes-ontap/task-administer-advanced-view.html["Administrar o Cloud Volumes ONTAP usando o Gerenciador de sistemas"^].

Apresentando agora uma forma mais simples de gerir licenças com a carteira digital BlueXP , pode experimentar uma gestão simplificada das licenças Cloud Volumes ONTAP utilizando pontos de navegação melhorados na carteira digital BlueXP :
...
Lista de VMs suportadas atualizadas para o Azure para se alinhar com as melhores práticas
...
11 de novembro de 2024 - fim da disponibilidade para licenças baseadas em nós
...
Remoção de implantações baseadas em nós do BlueXP  a opção de implantar sistemas Cloud Volumes ONTAP usando licenças baseadas em nós é obsoleta no BlueXP . Exceto em alguns casos especiais, você não pode usar licenças baseadas em nós para implantações do Cloud Volumes ONTAP para nenhum provedor de nuvem.
...
Adição de uma camada fria para dados do Cloud Volumes ONTAP no storage de Blobs do Azure
...
Suporte para o Cloud Volumes Service para o Google Cloud
...
7 de dezembro de 2020 Navegação entre o Cloud Manager e o Spot
...
18 de outubro de 2020 apresentando o serviço Compute
...
2 de fevereiro de 2025 novo suporte ao SO para o corretor de dados o corretor de dados agora é suportado em hosts executando Red Hat Enterprise 9,4, Ubuntu 23,04 e Ubuntu 24,04.
...
27 de outubro de 2024 - correções de erros
...
16 de setembro de 2024 atualizamos o serviço de cópia e sincronização do BlueXP  e o corretor de dados para corrigir alguns bugs. A nova versão do Data Broker é 1,0.55.

Consultor digital

12 de dezembro de 2024

Agora você pode visualizar o firmware de armazenamento, o firmware do SP/BMC e o Pacote de ransomware autônomo (ARP) que são recomendados para uma atualização. link:https://docs.netapp.com/us-en/active-iq/view-firmware-update-recommendations.html["Saiba como visualizar as recomendações de atualização de firmware"].

04 de dezembro de 2024

O AutoSupport widget do AutoSupport foi adicionado à tela principal do painel de controle para alertar os clientes sobre os problemas relacionados ao status do AutoSupport.

23 de setembro de 2024

A oferta de serviço do NetApp SupportEdge Basic agora inclui todos os recursos do consultor digital disponíveis no SupportEdge Advisor e no SupportEdge Expert, exceto a topologia de pilha completa (VMware), que não fornece visibilidade sobre o monitoramento de pilha completa da VMware, mesmo se ativada.
...
A página inicial da carteira digital tem um painel atualizado das suas licenças do NetApp e assinaturas do Marketplace, com a capacidade de detalhar serviços específicos, tipos de licença e ações necessárias.

A carteira digital da BlueXP  agora permite que você configure suas assinaturas para credenciais de provedor. Normalmente, você faz isso quando você se inscreve pela primeira vez em uma assinatura do Marketplace ou em um contrato anual. A alteração das credenciais da assinatura anteriormente só poderia ser feita na página credenciais.

Associar assinaturas a organizações agora você pode atualizar a organização à qual uma assinatura está associada diretamente da carteira digital.

Agora você gerencia as licenças do Cloud Volumes ONTAP ONTAP por meio da página inicial ou da guia *licenças de serviço de dados*. Use a guia *assinaturas do Marketplace* para exibir suas informações de assinatura.
...
Relatórios de uso aprimoramentos
...
30 de outubro de 2024 relatórios agora você pode gerar e baixar relatórios para ajudá-lo a analisar seu cenário. Os relatórios pré-projetados resumem failovers e failbacks, mostram detalhes de replicação em todos os locais e mostram os detalhes da tarefa nos últimos sete dias.
...
Teste gratuito de 30 dias agora você pode se inscrever para uma avaliação gratuita de 30 dias da recuperação de desastres do BlueXP . Anteriormente, as avaliações gratuitas duravam 90 dias.

Consulte a https://docs.netapp.com/us-en/bluexp-disaster-recovery/get-started/dr-licensing.html["Configure o licenciamento"].

Uma versão anterior incluiu atualizações para a estrutura de agendamento de teste de failover, que era necessária para suportar agendas diárias e semanais. Esta atualização exigiu que você desabilite e reative todos os planos de replicação existentes para que você possa usar as novas programações de teste de failover diárias e semanais. Este é um requisito único.
...
Ao criar um plano de replicação e mapear recursos de computação, agora é possível mapear pastas para que as VMs sejam recuperadas em uma pasta especificada para datacenter, cluster e host.
...
Quando você cria um plano de replicação, agora você pode ver o sistema operacional para cada VM no plano. Isso é útil para decidir como agrupar VMs em um grupo de recursos.

Para obter detalhes, https://docs.netapp.com/us-en/bluexp-disaster-recovery/use/drplan-create.html["Crie um plano de replicação"] consulte .

Quando você cria um plano de replicação, agora é possível adicionar um prefixo e sufixo aos nomes de VM no SIT de recuperação de desastres. Isso permite que você use um nome mais descritivo para as VMs no plano.

Para obter detalhes, https://docs.netapp.com/us-en/bluexp-disaster-recovery/use/drplan-create.html["Crie um plano de replicação"] consulte .

Limpar instantâneos antigos você pode excluir quaisquer snapshots que não sejam mais necessários além da sua contagem de retenção especificada. Os snapshots podem se acumular ao longo do tempo quando você diminui a contagem de retenção de snapshots, e agora você pode removê-los para liberar espaço. Você pode fazer isso a qualquer momento sob demanda ou quando você excluir um plano de replicação.
...
Suporte para datastores VMware VMFS on-premises a on-premises esta versão inclui suporte para VMs montadas em armazenamentos de dados do VMware vSphere Virtual Machine File System (VMFS) para iSCSI e FC protegidos para armazenamento no local. Anteriormente, o serviço fornecia uma _visualização de tecnologia_ com suporte a armazenamentos de dados VMFS para iSCSI e FC.
...
Suporte para e-Series agora você pode descobrir seus sistemas e-Series diretamente da BlueXP . Descobrir sistemas e-Series oferece uma visão completa dos dados em sua multicloud híbrida.
...
Se você tem ativos existentes e quer determinar se uma tecnologia precisa ser atualizada, você pode usar as opções de atualização da tecnologia de eficiência econômica da BlueXP . Você pode analisar uma breve avaliação de suas cargas de trabalho atuais e obter recomendações ou, se você enviou logs do AutoSupport para o NetApp nos últimos 90 dias, o serviço agora pode fornecer uma simulação de workload para ver como suas cargas de trabalho funcionam no novo hardware.
...
Capacidade de adicionar novos buckets e gerenciar buckets existentes do BlueXP  você teve a capacidade de visualizar os buckets do Google Cloud Storage no BlueXP  Canvas por um tempo. Agora você pode adicionar novos buckets e alterar propriedades para buckets existentes diretamente do BlueXP . https://docs.netapp.com/us-en/bluexp-google-cloud-storage/task-add-gcp-bucket.html["Veja como adicionar novos buckets do Google Cloud Storage"].
...
Não é mais possível criar um volume a partir de um modelo. Essa ação foi associada ao serviço de correção do BlueXP , que não está mais disponível.

Resiliência operacional

02 de abril de 2023

Usando o novo BlueXP  serviço de resiliência operacional da BlueXP  e as sugestões de correção de risco operacional de TI automatizada, você pode implementar soluções sugeridas antes que ocorra uma interrupção ou falha.
...
Detecte um comportamento anômalo do usuário usando o Data Infrastructure Insights Storage Workload Security
...
30 de setembro de 2024 agrupamento personalizado de cargas de trabalho de compartilhamento de arquivos com esta versão, agora você pode agrupar compartilhamentos de arquivos em grupos para facilitar a proteção do seu estado de dados. O serviço pode proteger todos os volumes de um grupo ao mesmo tempo. Anteriormente, você precisava proteger cada volume separadamente.
...
O FSX for ONTAP to Cloud Volumes ONTAP agora você pode replicar dados de um sistema de arquivos do Amazon FSX for ONTAP para o Cloud Volumes ONTAP.

https://docs.netapp.com/us-en/bluexp-replication/task-replicating-data.html["Saiba como configurar a replicação de dados"].

31 de Julho de 2022

Como fonte de dados, você agora pode replicar dados de um sistema de arquivos do Amazon FSX for ONTAP para os seguintes destinos ONTAP:
...
Agora você pode replicar dados de um sistema Cloud Volumes ONTAP ou de um cluster ONTAP no local para um sistema de arquivos do Amazon FSX for ONTAP ONTAP.
...
Atualização do ONTAP o serviço de atualizações de software da BlueXP  fornece uma experiência de atualização contínua aos usuários, mitigando riscos e garantindo que os clientes possam aproveitar totalmente os recursos do ONTAP.
...
Capacidade de analisar e aprovar o certificado de interface de gerenciamento do StorageGRID agora você tem a capacidade de analisar e aprovar um certificado de interface de gerenciamento do StorageGRID ao descobrir o sistema StorageGRID do BlueXP . Você também pode revisar e aprovar o certificado de interface de gerenciamento StorageGRID mais recente em uma grade descoberta.
...
Suporte para StorageGRID agora você pode descobrir seus sistemas StorageGRID diretamente do BlueXP . Ao descobrir o StorageGRID, você obtém uma visão completa dos dados na multicloud híbrida.

- Disposição em camadas

9 de Agosto de 2023

Use um prefixo personalizado para o nome do bucket no passado que você precisava usar o prefixo padrão "Fabric-pool" ao definir o nome do bucket, por exemplo, _Fabric-pool-bucket1_. Agora você pode usar um prefixo personalizado ao nomear seu bucket. Esse recurso está disponível somente ao categorizar dados no Amazon S3. https://docs.netapp.com/us-en/bluexp-tiering/task-tiering-onprem-aws.html#prepare-your-aws-environment["Saiba mais"].

Se você estiver usando vários conetores para gerenciar todos os BlueXP  sistemas de armazenamento em seu ambiente, alguns clusters nos quais você deseja implementar a disposição em camadas podem estar em diferentes conetores. Se você não tiver certeza de qual conetor está gerenciando um determinado cluster, poderá pesquisar em todos os conetores usando a disposição em camadas do BlueXP . https://docs.netapp.com/us-en/bluexp-tiering/task-managing-tiering.html#search-for-a-cluster-across-all-bluexp-connectors["Saiba mais"].

4 de Julho de 2023

Ajuste a largura de banda para transferir dados inativos quando você ativa a disposição em camadas do BlueXP , o ONTAP pode usar uma quantidade ilimitada de largura de banda da rede para transferir os dados inativos de volumes no cluster para o armazenamento de objetos. Se você notar que a disposição em categorias de tráfego está afetando as cargas de trabalho normais do usuário, poderá controlar a quantidade de largura de banda que pode ser usada durante a transferência. https://docs.netapp.com/us-en/bluexp-tiering/task-managing-tiering.html#changing-the-network-bandwidth-available-to-upload-inactive-data-to-object-storage["Saiba mais"].

Evento de disposição em camadas exibido na Central de notificações o evento de disposição em camadas "disponha dados adicionais do cluster <name> para o storage de objetos para aumentar a eficiência de storage" agora aparece como uma notificação quando um cluster está em camadas menos de 20% de seus dados inativos - incluindo clusters que não estão em categorias de dados.
...
Carga de trabalho de fábrica

02 de fevereiro de 2025

O CloudShell está disponível em qualquer lugar no console de fábrica da carga de trabalho do BlueXP  BlueXP . O CloudShell permite que você use as credenciais da AWS e do ONTAP fornecidas na sua conta do BlueXP  e execute os comandos da CLI da AWS ou da CLI do ONTAP em um ambiente semelhante ao shell.

link:https://docs.netapp.com/us-en/workload-setup-admin/use-cloudshell.html["Use o CloudShell"]

Atualização de permissões para bancos de dados a seguinte permissão está agora disponível no modo _read_ para bancos de dados: `iam:SimulatePrincipalPolicy`.
...
Agora, você pode visualizar as permissões que a fábrica de carga de trabalho do BlueXP  usa para executar várias operações, desde a descoberta de seus ambientes de storage até a implantação de recursos da AWS, como sistemas de arquivos em armazenamento ou bases de conhecimento para cargas de trabalho do GenAI BlueXP . Você pode visualizar as políticas e permissões do IAM para workloads de storage, bancos de dados, VMware e GenAI.
...
Suporte para contas de serviço na fábrica de carga de trabalho do BlueXP  as contas de serviço são agora suportadas na fábrica de carga de trabalho do BlueXP . Você pode criar contas de serviço para atuar como usuários de máquina que automatizam as operações de infraestrutura.
